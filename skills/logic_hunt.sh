...
JSON_PAYLOAD=$(jq -n \
	--arg model "$MODEL" \
	--arg system "You are the Logic Hunter. Find edge cases, null pointers, off-by-one errors, race conditions, and broken assumptions. Be precise. Only report real issues." \
...
while [ $attempt -lt $max_attempts ]; do
	...
	if [ "$HTTP_CODE" = "200" ]; then
		CONTENT=$(echo "$BODY" | jq -r '.choices[0].message.content // .choices[0].message.reasoning_content // empty' 2>/dev/null || echo "")
